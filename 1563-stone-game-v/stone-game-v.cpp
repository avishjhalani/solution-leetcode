...
public:
    ...
    int solve(int left, int right, vector<int>& prefixSum) {
        ...
        for (int mid = left; mid < right; mid++) {
            ...
            if (leftsum < rightsum) {
                score = max(score, leftsum + solve(left, mid, prefixSum));
            } else if (leftsum > rightsum) {
                score = max(score, rightsum + solve(mid + 1, right, prefixSum));
            } else {
                score = max(score, max(leftsum + solve(left, mid, prefixSum),
